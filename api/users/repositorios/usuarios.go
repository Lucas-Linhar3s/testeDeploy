...
// Buscar traz todos os usuários que atendem um filtro de nome ou email
func (repositorio Usuarios) Buscar(nomeOuNick string) ([]modelos.Usuario, error) {
	nomeOuNick = fmt.Sprintf("%%%s%%", nomeOuNick) // %nomeOuNick%

	linhas, erro := repositorio.db.Query(
		"select id, nome, email, dt_cadastro, dt_atualizacao from usuarios where nome LIKE ? or email LIKE ?",
...
func (repositorio Usuarios) BuscarPorID(ID uint64) (modelos.Usuario, error) {
	linhas, erro := repositorio.db.Query(
		"select id, nome, email, dt_cadastro, dt_atualizacao from usuarios where id = ?",
		ID,
	)
	if erro != nil {
